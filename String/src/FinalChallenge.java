...
public class FinalChallenge {
	public static void main(String[] args) {
		...
		System.out.println(csvToJson(entrada));
	...
	public static String csvToJson(String[] videos) {
		...
		for (int i = 0; i < videos.length; i++) {
			String[] list = videos[i].split(",");
			String id = list[0];
			String title = list[1].substring(6).replace(".mp4", "");
			int duration = Integer.parseInt(list[2]);
			Video video = new Video(id, title, duration);
			saida = saida + String.format(
					"""
					\t{
					\t  "id": "%s",
					\t  "title": "%s",
